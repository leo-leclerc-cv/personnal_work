function boardBattleEngine (characterPlaying, key) {
    ...
    //Si n'est pas bloqué -->
    	...
    	bump++;
    	document.getElementById("info_container").insertAdjacentHTML("beforeEnd", '<audio autoplay id="bump_n°'+bump+'"><source src="public/audio/Bump.ogg" type="audio/ogg">Your browser does not support the audio tag.</audio>');
...
document.addEventListener("keydown", function (e) {
	if (victory==false) {
		...
		if (Number(document.getElementById("points_P1").textContent)<=50&&Number(document.getElementById("points_P2").textContent)<=50) {
			...
			document.getElementById("info_container").insertAdjacentHTML("beforeEnd", '<audio autoplay><source src="public/audio/Woah!.ogg" type="audio/ogg">Your browser does not support the audio tag.</audio>');
		}
	}
	else {
